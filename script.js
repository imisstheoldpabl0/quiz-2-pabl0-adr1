...
async function quiz() { // Se debería llamar quiz porque al final estamos llamando a la funcion quiz no?
    ...
    document.getElementById("quiz").innerHTML =
        `<fieldset>
            ...
            <input type="radio" name="quiz" id="answer2" value="${respuestas_incorrectas[0][0]}">
            <label for="id=" answer3" id="answer3">${respuestas_incorrectas[0][1]}</label>
            <input type="radio" name="quiz" id="answer3" value=${respuestas_incorrectas[0][1]}>
            <label for="id=" answer4" id="answer4">${respuestas_incorrectas[0][2]}</label>
            <input type="radio" name="quiz" id="answer4" value=${respuestas_incorrectas[0][2]}>
        </fieldset>`
    ...
    for (let i = 0; i < labels.length; i++) {
        labels[i].addEventListener("click", function () {
            respuestas.push(document.getElementById(`answer1.value`)) // Aquí de momento sólo funciona con la respuesta 1, sería conveniente darles a todos los inputs el name del fieldset para a cada click directamente recoger el value del input seleccionado
